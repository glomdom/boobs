...
void vulkan_command_buffer_allocate(
    ...
    boobs_zero_memory(out_command_buffer, sizeof(out_command_buffer));
...
void vulkan_command_buffer_end_single_use(
    ...
    VK_CHECK(vkQueueWaitIdle(queue));   // wait until command buffer commands have been executed so we can free
